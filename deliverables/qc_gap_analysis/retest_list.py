...
the release certificates they supersede.
    ...
    python3 deliverables/qc_gap_analysis/retest_list.py
        -> tracker/CoQ_Retest_List_<today>.xlsx / .csv / .md    the 83 retest certificates
        -> tracker/CoQ_Initial_List_<today>.xlsx / .csv / .md   the 83 release certificates, each with
                                                                the retest certificate that supersedes it
...
ICOLS = ["Tranche", "Release CoQ code", "Issued", "Superseded by (retest CoQ)", "Retest issued", "Production batch (P lot)", "Cultivation batch",
         "Strain", "Product type", "Grade", "Product code", "Potency window (%)", "Nominal ± tol (%)",
         "Total THC result (%)", "CBD (%)", "CBN (%)", "Potency certificate", "Certificate issued", "Laboratory",
         "Internal CoA", "Internal CoA issued", "Retest Total THC (%)", "Retest potency certificate"]


def lotkey(c):
    return re.sub(r"[\s\-/_.＊*]+", "", (c.get("pp") or c.get("cb") or "")).upper()


def initial_rows():
    """The release certificate of every lot that has a retest in tranches 1–3, with the retest
    ...
    for c in d["coqs"]:
        ...
        if not rt:
            continue
        r4 = next((r for r in c["rows"] if r["no"] == "4"), {}); r5 = next((r for r in c["rows"] if r["no"] == "5"), {}); r6 = next((r for r in c["rows"] if r["no"] == "6"), {})
        t4 = next((r for r in rt["rows"] if r["no"] == "4"), {})
        spc = c.get("spc") or rt.get("spc") or {}
        ...
        t = tr.get(rt.get("regcode") or "") or tr.get(c.get("pp") or "") or tr.get(c.get("cb") or "")
        out.append({
            "Tranche": t, "Release CoQ code": code, "Issued": c.get("issue") or "",
            "Superseded by (retest CoQ)": rt.get("regcode") or "", "Retest issued": rt.get("issue") or "",
            ...
            "Retest Total THC (%)": (t4.get("res") or "").replace("%", "").strip(), "Retest potency certificate": t4.get("doc") or ""})
    def key(r):
        m = re.search(r"_26-(\d+)$", r["Release CoQ code"])
...
def write(R, base, COLS=COLS, title="Certificates of quality — 12-month retest, tranches 1, 2 and 3", sheetname="Retest CoQs — all"):
    ...
    def sheet(ws, rows_):
        ...
        widths = {"Tranche": 8, "CoQ code": 15, "Release CoQ code": 15, "Superseded by (retest CoQ)": 16, "Retest issued": 11, "Retest Total THC (%)": 12, "Retest potency certificate": 16, "Issued": 11, "Supersedes": 15, "Superseded issued": 11, "Production batch (P lot)": 12, "Cultivation batch": 14,
    ...
    ws = wb.active; ws.title = sheetname; sheet(ws, R)
    for t in ("1", "2", "3"):
...
def main(argv):
    ...
    I = initial_rows(); base = os.path.join(HERE, "tracker", "CoQ_Initial_List_%s" % dt.date.today().isoformat())
    write(I, base, ICOLS, "Certificates of quality — release round of the tranche 1, 2 and 3 batches, and the retest certificate that supersedes each", "Release CoQs — all")
    print("release certificates with a retest:", len(I), dict(collections.Counter(r["Tranche"] for r in I)))
