...
ITEMS = [
    ...
    ("OI-41", "Document content", "ruled",
     "Closed 21.09.2026 — it was never a missing result; it was a stale status, and 316 cells were printing nothing because of it",
     "Determinations #1 Identification A, #2 Identification B and #7 Foreign matter are "
     "performed in the Purely Plant laboratory on every batch, and the batch's own internal "
     "certificate of analysis certifies all three. On **111** of the 172 certificates the "
     "three printed NOTHING — 316 cells — and this item spent three weeks asking which "
     "documents were missing. None were. Every one of those rows already carried "
     "\"Conforms | \u041e\u0434\u0433\u043e\u0432\u0430\u0440\u0430\" and already cited its own issued, numbered "
     "iCoA, put there by export_coq_artifact_data.py where the iCoA register is read. What "
     "the row kept was the STATUS build_coq_schedule.status_of assigns a determination with "
     "no document in the candidate pool AT SCHEDULE TIME — ST_ICOA, \"to be performed \u2014 "
     "see route\" \u2014 because the iCoA is attached later, at export, and nothing went back "
     "to correct the status. The renderer reads the STATUS, not the result "
     "(coq_build.js `cell()`), so the certificate printed an empty marker for a "
     "determination whose certificate is issued, numbered and signed.",
     "Owner, 21.09.2026: \"Ident A and Ident B and even the Foreign Matter parameters are "
     "contained in the iCoA for each CoQ, this is known.\" They were. "
     "apply_icoa_status.py sets every such row to `covered` and drops the route with it, at "
     "the one layer that can see both the citation and the register \u2014 305 cells on 111 "
     "certificates. Three certificates (CoQ-PP_26-071 JD012603/01, -166 CC042601, -167 "
     "FB042601) had no result on the row at all; their iCoA register entries state what "
     "their internal certificates say \u2014 \"Conforms\" for two, CNP \u041f\u041f\u041a26111 for the third "
     "\u2014 so those 9 cells are LIFTED from the register in the spelling the other 169 already "
     "use, the CNP one citing the CNP certificate directly per the ruling of 20.09.2026. "
     "All 172 certificates now print #1, #2 and #7. The same rule is applied at the source "
     "in export_coq_artifact_data.py, so a full chain replay converges. A new hard assertion "
     "in coq_check.js makes the regression impossible to ship: a certificate that prints "
     "nothing for #1, #2 or #7 FAILS the build.",
     "Nothing is outstanding. What remains beside it is a different question: 12 initial "
     "certificates still carry ST_ICOA for #3 Identification C, which is routed to the "
     "cannabinoid assay certificate rather than to the in-house laboratory, and is covered "
     "wherever that certificate exists.",
     "apply_icoa_status.py; export_coq_artifact_data.py; build_coq_schedule.ST_ICOA; "
     "icoa_handoff/v3/icoa_v44_recs.json; design_handoff/toolchain/coq_check.js A11; "
