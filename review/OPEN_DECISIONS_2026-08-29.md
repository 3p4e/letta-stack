...
|---|---|---|---|
| **B5** | `FB012601` | Certificate ППК26067 prints `/1` on two independent readings. FB012601 is a 2026 batch, a vintage that never registers a parent. The bare register row is the anomaly. | **Correct the register row to `FB012601/1`** — unless an unregistered sub-lot genuinely exists, in which case register it. Question of fact settled; only the ruling remains. |
