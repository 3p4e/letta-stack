...
Runs the SKILL.md §6 checks on any produced .docx and prints a PASS/FAIL report:
  ...
  * glyphs     : every run's declared font can actually render that run's text, and
                 the render environment has no subset font shadowing a full face
                 (catches Macedonian scrambled into tofu — see pp_assets.py)
...
def main():
    ...
    if pp_assets is not None:
        env = pp_assets.check_environment()
        bad = pp_assets.audit_docx(a.docx)
        print(f"   render environment {'OK' if not env else 'FAIL'}")
        for e in env:
            print(f"     ! {e}")
        print(f"   glyph coverage {'OK' if not bad else 'FAIL'}")
        for face, miss, sample in bad:
            print(f"     ! {face} cannot render {miss!r}  e.g. {sample!r}")
        ok &= (not env) and (not bad)
    else:
