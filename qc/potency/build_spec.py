...
LBLU="DDEBF7";NAVY="2B547E";LGREY="F7FAFC";WHITE="FFFFFF";AMBER="FFF2CC";GREEN="E2EFDA";ROSE="FCE4D6"
OUT="PP_Potency_MASTER_Spec.xlsx";CAP=0.10;U=0.05;EPS=1e-9
TODAY=dt.date(2026,8,31)
def band(n): t=n*CAP; return t,n-t,n+t-0.01
LAD=list(range(28,7,-2));ROMAN=["I","II","III","IV","V","VI","VII","VIII","IX","X","XI"]
# grade is assigned PER STRAIN below, not from a company-wide ladder
...
# PER-STRAIN grading: within each strain the strongest class is Grade I, descending
_by={}
for _s,_c,_p,_a,_n,_t in B: _by.setdefault(_s,set()).add(_n)
GRADE={}
for _s,_ns in _by.items():
    for _i,_n in enumerate(sorted(_ns,reverse=True)): GRADE[(_s,_n)]=ROMAN[_i]

ABBR={"Amnesia Core Cut":"ACC","Blue Gelato":"BG","Blue Sunset Sherbet":"BSS","Cap Junky":"CJ",
"Cash Cow":"CC","Chem Flyer":"CF","Clemosa A Bud":"CLE","Fat Bastard":"FB","Gorilla Glue":"GG",
"Grape Pie":"GP","Grapes and Cream":"GRC","High Pro Amnesia":"HPA","Jelly Donuts":"JD",
"Jokerz 31":"J31","Kush Crasher":"KC","Motor Breath":"MB","Orange Punch Mimosa":"OPM",
"Permanent Marker":"PM","Pure Michigen":"PUM","Scrambler":"SCR","Sleepy Joe":"SJ",
"Wedding Crusher":"WC"}
def spec(s,g): return f"QCSP_001_{ABBR[s]}-{g}_v.01"
def ptype(s,n): return f"{ABBR[s]}_THC{n}:CBD1"
...
for s,cul,pp,a,n,tr in B:
    ...
    rows.append(dict(strain=s,cultiv=cul,pp=pp,assay=a,nom=n,tol=t,lo=lo,hi=hi,tr=tr,
                     g=GRADE[(s,n)],ab=ABBR[s],spec=spec(s,GRADE[(s,n)]),pt=ptype(s,n),ml=a-lo,mu=hi-a,h=hd,p=pd_,hs=hs,ps=ps,
...
h=["#","Tranche","Strain","Grade","Class","Nominal value %","Tolerance","Potency range",
   ...
   "Specification doc. code","Product type","Manufacturing (harvest) date","Packaging date",
   "Harvest→pack (days)","Age since packaging (days)"]
...
for i,x in enumerate(sorted(rows,key=lambda r:(-r["nom"],-r["assay"])),start=1):
    ws.append([i,f"T{x['tr']}",x["strain"],f"Grade {x['g']}",f"THC {x['nom']} / CBD 1",
               ...
               f"{x['assay']:.2f}%",round(x["ml"],2),round(x["mu"],2),x["cultiv"],x["pp"],x["spec"],x["pt"],
               fd(x["h"]),fd(x["p"]),x["cure"] if x["cure"] is not None else "—",x["age"]])
    for c in range(1,len(h)+1):
        sc(ws.cell(row=r,column=c),fill=LGREY if r%2==0 else None,
           align="left" if c==3 else "center",bold=(c in (4,6,7,8,14,15,16,17)))
    ...
    sc(ws.cell(row=r,column=14),fill=LBLU,bold=True)
    sc(ws.cell(row=r,column=15),fill=LBLU,bold=True)
    if x["h"] is None: sc(ws.cell(row=r,column=16),fill=ROSE,bold=True)
    r+=1
for c,w in enumerate([5,9,20,10,16,14,11,20,12,16,17,15,13,24,22,26,16,18,22],start=1):
...
h2=["Strain","Grade","Class","Nominal value %","Tolerance","Potency range",
    "Specification doc. code","Product type","PP Batch No.","Exact THC %",
    "Manufacturing (harvest) date","Packaging date","Tranche"]
...
for s in sorted(bs,key=str.lower):
    ...
    for n in sorted(bs[s],reverse=True):
        ...
        for m in mem:
            ws2.append([s if firsts else "",f"Grade {GRADE[(s,n)]}" if firstg else "",
                        f"THC {n} / CBD 1" if firstg else "",f"{n:.2f}%" if firstg else "",
                        f"± {t:.2f}%" if firstg else "",
                        f"{lo:.2f}% – {hi:.2f}%" if firstg else "",
                        m["spec"] if firstg else "", m["pt"] if firstg else "",
            ...
            if firstg:
                ...
                sc(ws2.cell(row=r,column=7),fill=LBLU,bold=True)
                sc(ws2.cell(row=r,column=8),fill=LBLU,bold=True)
            if m["h"] is None: sc(ws2.cell(row=r,column=11),fill=ROSE,bold=True)
            firsts=False;firstg=False;r+=1
for c,w in enumerate([20,10,16,14,11,20,24,22,13,12,26,16,9],start=1):
...
txt=[("1","Where the source gives a date range or a pair, the FIRST date is taken — for both "
 ...
 ("2","Grade numbering is PER STRAIN: within each strain the strongest class it holds is "
      "Grade I, the next one down Grade II, and so on. Grade I therefore means different absolute "
      "potencies on different strains — it means that strain\u2019s strongest."),
